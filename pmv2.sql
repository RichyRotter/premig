--------------------------------------------------------
--  Datei erstellt -Freitag-Oktober-03-2025   
--------------------------------------------------------
--------------------------------------------------------
...
*/
  ...
  TYPE rule_counter_record IS RECORD (
    rule_nr   NUMBER,
    cnt       NUMBER
  );

  TYPE rule_counter_table IS TABLE OF rule_counter_record INDEX BY PLS_INTEGER;
  v_rule_counters rule_counter_table;



  v_rule_result         varchar2(260);
  v_error               number := 0;
  dcnt                  number := 0;
  v_exa                 number := 0;
  v_adb                 number := 0;
...
-- prod dbs
   cursor c_migdb is
       select db_primkey
       from mig_database
       where tranche = F_ACTIVE_TRANCHE
       and environment = 'P'
       and frozen ='N'
  --     and db_primkey=100977
       ;
...
-- reset
  ...
  update mig_database
         set target_platform = 'TBD',
             ...
             justification = null 
  where frozen ='N'
  and tranche = F_ACTIVE_TRANCHE;

update mig_database
       set BUSINESSAPPL = (SELECT "BusinessAppl"
                          FROM v_modernex_1st_scope
                          where lower (v_modernex_1st_scope."OracleDB") =
                                lower (mig_database.oracledb)
                                fetch first row only)
  where frozen ='N'
  and tranche = F_ACTIVE_TRANCHE;
...
from mig_database
  where frozen ='N'
  and tranche = F_ACTIVE_TRANCHE
  and BUSINESSAPPL is null;
  
if v_error > 0 then
    RAISE_APPLICATION_ERROR(-20019, 'Missing business appl errcnt='|| v_error);
end if;


 -- Initialize counters
  FOR r IN c_active_rules_all LOOP
    v_rule_counters(r.rule_nr).rule_nr := r.rule_nr;
    v_rule_counters(r.rule_nr).cnt := 0;
  END LOOP;

-- work on dbs
  FOR d IN c_migdb LOOP
        dcnt := dcnt + 1;
        v_rule_result := 'not applied';

        -- Durchlauf über aktive Regeln
        FOR r IN c_active_rules LOOP
            IF v_rule_result != 'Rule applied' THEN
                v_rule_result := F_EXEC_CLASS_RULE(
                    d.db_primkey,
                    r.rule_nr,
                    v_rule_counters(r.rule_nr).cnt
                );

                -- Fehler zählen
                IF substr(v_rule_result,1,6) = 'ERROR:' THEN
                    v_error := v_error + 1;
                END IF;
            END IF;
        END LOOP;
       ...
       FOR rf IN c_active_rules_fix LOOP
            v_rule_result := F_EXEC_CLASS_RULE(
                    d.db_primkey,
                    rf.rule_nr,
                    v_rule_counters(rf.rule_nr).cnt
                );
                -- Fehler zählen
            IF substr(v_rule_result,1,6) = 'ERROR:' THEN
                    v_error := v_error + 1;
        ...
        -- Commit nach jeder DB (optional)
     COMMIT;
        
        -- Logge alle 20 DBs
        IF MOD(dcnt, 20) = 0 THEN
            p_log('mig', 'anz dbs progressed ' || dcnt);
        END IF;
    END LOOP;
    p_log('mig', 'anz dbs progressed ' || dcnt);
 ...
 update mig_database
         set target_platform = 'ATPS',
             justification = 'no Prod DB in trache ' 
  where frozen ='N'
  and tranche = F_ACTIVE_TRANCHE
...
FOR r IN c_active_rules_all LOOP
  IF v_rule_counters.EXISTS(r.rule_nr) THEN
    p_log('mig', 'Rule ' || r.rule_nr || ' count = ' || v_rule_counters(r.rule_nr).cnt);
  ELSE
    p_log('mig', 'Rule ' || r.rule_nr || ' count = 0');
  END IF;
END LOOP;

select count(*) into v_exa from mig_database where target_platform = 'EXAD' and tranche = F_ACTIVE_TRANCHE;
select count(*) into v_adb from mig_database where target_platform = 'ATPS' and tranche = F_ACTIVE_TRANCHE;
select count(*) into v_tbd from mig_database where target_platform = 'TBD' and tranche = F_ACTIVE_TRANCHE;

p_log ('mig','Anz exa: ' || to_char(v_exa));
p_log ('mig','Anz adb: ' || to_char(v_adb));
p_log ('mig','Anz tbd: ' || to_char(v_tbd));

p_log ('mig','End classify mig_database ERRORS='||v_error);

  update mig_control set status = 'finished errcnt='||v_error
  where nr = 25;
  commit;
...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "MIGRATIONV2"."P_COMPLETE_DEPLOYMENT" 
(
  load_em_data IN VARCHAR2 default 'N'
)
AS 
 v_ret number;
BEGIN
  ...
  delete from mig_log;
  commit;
  p_start_calculation (load_em_data);
  p_deploy_exad_infra;
  p_prov_adb;
  commit;
...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "MIGRATIONV2"."P_DEBUG" 
(
  P_DBG_ACTIVE IN VARCHAR2 
, P_DBG_INFO IN VARCHAR2 
) AS 
...
BEGIN
  ...
  if P_DBG_ACTIVE = 'Y' then
    v_number := s1.nextval;
    insert into mig_zz_debug values (v_number, p_dbg_info, Null);
    commit;

    LOOP
         DBMS_LOCK.SLEEP(3);
        DBMS_OUTPUT.PUT_LINE('Looping...');
        select dbg_action into v_action
        from mig_zz_debug
        where dbg_nr = v_number;
        
        if v_action = 'a' then
          RAISE_APPLICATION_ERROR(-20001, 'Abort due to action a');
        end if;
        EXIT WHEN v_action = 'c';
    END LOOP;
  end if;
END P_DEBUG;
 ...
 */   
 ...
 update mig_database set db_target_guid = (select target_guid
                                           from mig_em_metric_db_hrs
                                           where target_name = lower(oracledb)
                                           fetch first row only)
;
...
INSERT INTO mig_infran_param_rules VALUES (1,  'EXA-D fixstarter',                             'Y', 10);
INSERT INTO mig_infran_param_rules VALUES (2,  'Gold(+) to EXAD',                              'N', 20);
INSERT INTO mig_infran_param_rules VALUES (3,  'ATPS low fruits',                              'Y', 30);
INSERT INTO mig_infran_param_rules VALUES (4,  'Big-Dbs to EXAD',                              'Y', 40);
INSERT INTO mig_infran_param_rules VALUES (5,  'High-io to EXAD',                              'Y', 50);
INSERT INTO mig_infran_param_rules VALUES (6,  'max ecpu to EXAD (before split)',              'Y', 60);
INSERT INTO mig_infran_param_rules VALUES (7,  'Exceeds infra limit',                          'Y', 70);
INSERT INTO mig_infran_param_rules VALUES (8,  'ATPS ecpu/memory ratio rule',                  'Y', 80);
INSERT INTO mig_infran_param_rules VALUES (9,  'ATPS ecpu/session ratio rule',                 'Y', 90);
INSERT INTO mig_infran_param_rules VALUES (10, 'ATPS min ecpu rule',                           'Y', 100);
INSERT INTO mig_infran_param_rules VALUES (11, 'ATPS read iops/storage ratio rule',            'N', 110);
INSERT INTO mig_infran_param_rules VALUES (12, 'ATPS write iops/storage ratio rule',           'N', 120);
INSERT INTO mig_infran_param_rules VALUES (13, 'ATPS min storage',                             'Y', 130);
INSERT INTO mig_infran_param_rules VALUES (14, 'ATPS final ecpu/memory ruöe',                  'Y', 140);
INSERT INTO mig_infran_param_rules VALUES (15, 'CPAT complex',                                 'N', 150);
INSERT INTO mig_infran_param_rules VALUES (16, 'DG for ATPS/EXAD',                             'Y', 160);
INSERT INTO mig_infran_param_rules VALUES (20, 'Left over for ATP',                            'Y', 1001);
INSERT INTO mig_infran_param_rules VALUES (19, 'Application dependency',                       'Y', 1002);
INSERT INTO mig_infran_param_rules VALUES (18, 'RAC on EXA only Prod',                         'N', 1003);
INSERT INTO mig_infran_param_rules VALUES (17, 'RAC on EXA for ALL',                           'Y', 1004);
INSERT INTO mig_infran_param_rules VALUES (21, 'DG f. Gold',                                   'Y', 1005);
INSERT INTO mig_infran_param_rules VALUES (22, 'DG f. Silver+',                                'Y', 1006);
INSERT INTO mig_infran_param_rules VALUES (23, 'DG f. Silver',                                 'Y', 1007);
...
MERGE INTO mig_database md
USING (
    SELECT
        LOWER(target_name) AS target_name,
        MAX(CASE WHEN metric_column = 'sga_total'     THEN maximum END) AS sga_gb,
        MAX(CASE WHEN metric_column = 'pga_total'     THEN maximum END) AS pga_gb,
        MAX(CASE WHEN metric_column = 'ALLOCATED_GB'  THEN maximum END) AS alloc_gb,
        MAX(CASE WHEN metric_column = 'USED_GB'       THEN maximum END) AS used_gb
    FROM v_mig_em_latest_mem_sstorage_metrics
    GROUP BY LOWER(target_name)
) metrics
ON (
    LOWER(md.oracledb) = metrics.target_name
    AND md.frozen = 'N'
)
WHEN MATCHED THEN UPDATE SET
    md.sga_gb   = NVL(metrics.sga_gb,   md.sga_gb),
    md.pga_gb   = NVL(metrics.pga_gb,   md.pga_gb),
    md.alloc_gb = NVL(metrics.alloc_gb, md.alloc_gb),
    md.used_gb  = NVL(metrics.used_gb,  md.used_gb);
...
WHERE d.sga_gb IS NULL
  ...
  AND EXISTS (
    ...
    UPDATE mig_database
    SET sga_gb = ROUND(sga_gb/1024, 3),
        pga_gb = ROUND(pga_gb/1024, 3),
        ...
        step_marker := 14;   
          ...
          FOR metric_index IN 1..metrics.COUNT LOOP
            ...
            IF return_value <> 0 THEN
                ...
                p_log ('mig','metric error '||db_record.db_target_guid);
            END IF;
        END LOOP;
        step_marker := 2;   

       --P_debug (v_debug, step_marker);
...
set define off;
  ...
  CREATE OR REPLACE EDITIONABLE PROCEDURE "MIGRATIONV2"."P_START_CALCULATION" 
(
  load_em_data IN VARCHAR2 default 'N'
)
AS
...
BEGIN
   -- reset cintrl table
    ...
    if load_em_data = 'Y' then
       P_LOAD_EM_DATA;
    else
       p_log('mig', 'Step1: skip load EM data');
    end if;
    commit;
...
BEGIN
    ...
    FOR xdb IN c_mdb LOOP
        ...
        v_info := 'ecpus='||xdb.anz_ecpus||' mem p. ecpu='||v_limit_rec.gb_mem_per_ecpu||' = max GB='||round(xdb.anz_ecpus * v_limit_rec.gb_mem_per_ecpu,3)||' sga + pga = '||round(xdb.sga_gb + xdb.pga_gb,3);
        -- cechk
        
        if v_plan_value < v_calc_value then
            if (v_calc_value/v_plan_value) > v_limit_rec.ATPS_EXEED_TOLERANZ_FACTOR then
               -- too expensive
                v_ret := store_limit(xdb.db_target_guid, 'ATPS', 'ecpu/mem ratio ',v_plan_value,v_calc_value,8,v_info);
            else
               -- increase
        ...
        BEGIN
           ...
           if v_plan_value < v_calc_value then
             if (v_calc_value/v_plan_value) > v_limit_rec.ATPS_EXEED_TOLERANZ_FACTOR then
               -- too expensive
                v_ret := store_limit(xdb.db_target_guid, 'ATPS', 'ecpu/session ratio ',v_plan_value,v_calc_value,9,v_info);
             else
                -- increase
                v_plan_value := v_calc_value;
             end if; 
            ...
            BEGIN
                ...
                IF v_test < v_limit_rec.read_iops_per_gb THEN
                    v_ret := store_limit(xdb.db_target_guid, 'ATPS', 'read iops/gb ratio '
                                                                                 ...
                                                                                 || '/'
                                                                                 || to_char(xdb.alloc_gb), v_limit_rec.read_iops_per_gb, v_test,11,v_info);
            ...
            BEGIN
                ...
                IF v_test > v_limit_rec.write_iops_per_gb THEN
                    v_ret := store_limit(xdb.db_target_guid, v_limit_rec.target_config, 'writes iops/gb ratio '
                                                                                 ...
                                                                                 || '/'
                                                                                 || to_char(xdb.alloc_gb), v_limit_rec.read_iops_per_gb, v_test,12,v_info);
...
FROM
    ...
    and  "OracleDB"   not in ('iuclida6_pdeluh1000023')      
    ;
