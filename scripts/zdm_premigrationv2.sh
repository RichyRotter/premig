...
pre_checks() {
	...
	if [ "X${p_dbmp_primkey}" != "X" ]; then
                     
		load_app_data ${p_dbmp_primkey}
...
EOF
	...
	if [ "X$dbmp_nfs_ip" != "X" ]; then
	  ...
	  #  nfs_mountp=$dbmp_nfs_mp
        ...
        case $nfs_ip in
		10.127.123.212) nfs_mountp="${nfs_mountp}2" ;;
		10.127.123.213) nfs_mountp="${nfs_mountp}3" ;;
		             *) mix=0 ;;
	esac
...
while getopts "o:b:j:d:t:a:z:c:n:i:r:m:x:p:l:" opt; do
	case $opt in
	d) srcdb="$OPTARG" ;;
	t) runTest="$OPTARG" ;;
	a) appnr="$OPTARG" ;;
