...
--------------------------------------------------------
--------------------------------------------------------
--  DDL for Function F_ACTIVE_TRANCHE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "MIGRATIONV2"."F_ACTIVE_TRANCHE" RETURN VARCHAR2 AS 

  v_tranche varchar2(20);

BEGIN

  select mig_tranche into v_tranche
  from mig_tranchen
  where mig_tr_active = 'Y';
  
  return v_tranche;
  
EXCEPTION
    WHEN OTHERS THEN
        p_log ('mig','f_active_tranche: An error occurred: ' || SQLERRM);
        RETURN NULL;
END F_ACTIVe_TRANCHE;
...
BEGIN
     ...
     execute IMMEDIATE ('truncate table mig_em_metric_db_hrs');
      ...
      begin
        ...
        value (
         SELECT target_guid, lower(target_name), target_type, metric_label,
...
/
--------------------------------------------------------
--  DDL for Function F_EXEC_CLASS_RULE
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE FUNCTION "MIGRATIONV2"."F_EXEC_CLASS_RULE" (
    P_DB_PRIMKEY IN NUMBER,
    P_RULE_NR    IN NUMBER,
    P_CNT        IN OUT NUMBER
) RETURN VARCHAR2 AS

    v_return       VARCHAR2(256) := 'NOT APPLIED';
    v_limits       mig_infran_param_technical_limits%ROWTYPE;
...
BEGIN
    ...
    CASE P_RULE_NR
        ...
        WHEN 1 THEN
            UPDATE mig_database
               SET target_platform = 'EXAD',
                   justification   = justification||' : '||'Rule1: EXA-D Fixstarter'
             WHERE db_primkey = P_DB_PRIMKEY
               AND LOWER(oracledb) IN (
                   SELECT LOWER(oracledb)
                     FROM mig_infran_param_exad_fixstarter
               );

        WHEN 2 THEN
            UPDATE mig_database
               SET target_platform = 'EXAD',
                   justification   = justification||' : '||'Rule2: Service-Category Gold'
             WHERE db_primkey = P_DB_PRIMKEY
               AND service_category LIKE 'GOL%';

        WHEN 3 THEN
            UPDATE mig_database
               SET target_platform = 'ATPS',
                   justification   = justification||' : '||'Rule3: ATPS Low Fruit'
             WHERE db_primkey = P_DB_PRIMKEY
               AND db_primkey IN (SELECT db_primkey FROM mig_cpat_low_fruits);

        WHEN 4 THEN
            UPDATE mig_database
               SET target_platform = 'EXAD',
                   justification   = justification||' : '||'Rule4: Big DB (Storage > Limit)'
             WHERE db_primkey = P_DB_PRIMKEY
               AND alloc_gb > v_limits.adb_max_storage;

        WHEN 5 THEN
            UPDATE mig_database
               SET target_platform = 'EXAD',
                   justification   = justification||' : '||'Rule5: High-IO (Service Category)'
             WHERE db_primkey = P_DB_PRIMKEY
               AND service_category LIKE 'GOL%';
        ...
        WHEN 6 THEN
            UPDATE mig_database
               SET target_platform = 'EXAD',
                   justification   = justification||' : '||'Rule6: eCPU > Limit'
             WHERE db_primkey = P_DB_PRIMKEY
               AND anz_ecpus > v_limits.adb_max_ecpu;

        WHEN 7 THEN
            UPDATE mig_database
               SET target_platform = 'EXAD',
                   justification   = justification||' : '||'Rule7: Exceeds Infra Limit'
             WHERE db_primkey = P_DB_PRIMKEY
               AND db_target_guid IN (
                   SELECT db_target_guid
                     FROM mig_calc_db_exeeds_limit
                    WHERE target_config = 'ATPS'
               );
        ...
        WHEN 8 THEN
            UPDATE mig_database
               SET target_platform = 'ATPS',
                   justification   = justification||' : '||'Rule8: eCPU/Memory Ratio OK'
             WHERE db_primkey = P_DB_PRIMKEY
               AND (sga_gb + pga_gb) / anz_ecpus >= v_limits.gb_mem_per_ecpu
               AND anz_ecpus > 0
        ...
        WHEN 9 THEN
            ...
            IF v_metric_value / v_limits.sessions_per_ecpu > v_anz_ecpus THEN
                UPDATE mig_database
                   SET target_platform = 'EXAD',
                       justification   = justification||' : '||'Rule9: eCPU/Sessions Ratio Exceeded'
                 WHERE db_primkey = P_DB_PRIMKEY
                   AND target_platform = 'TBD';
            END IF;
        ...
        WHEN 10 THEN
            UPDATE mig_database
               SET target_platform = 'ATPS',
                   justification   = justification||' : '||'Rule10: Min eCPU OK'
             WHERE db_primkey = P_DB_PRIMKEY
               AND anz_ecpus >= v_limits.min_ecpu
               AND target_platform = 'TBD';
        ...
        WHEN 11 THEN
            ...
            IF v_plan_value > 0 AND (v_metric_value / v_plan_value) < v_limits.read_iops_per_gb THEN
                UPDATE mig_database
                   SET target_platform = 'EXAD',
                       justification   = justification||' : '||'Rule11: Read IOPS/GB too low'
                 WHERE db_primkey = P_DB_PRIMKEY
                   AND target_platform = 'TBD';
            END IF;
        ...
        WHEN 12 THEN
            ...
            IF v_plan_value > 0 AND (v_metric_value / v_plan_value) > v_limits.write_iops_per_gb THEN
                UPDATE mig_database
                   SET target_platform = 'EXAD',
                       justification   = justification||' : '||'Rule12: Write IOPS/GB too high'
                 WHERE db_primkey = P_DB_PRIMKEY
                   AND target_platform = 'TBD';
            END IF;
        ...
        WHEN 13 THEN
            UPDATE mig_database
               SET target_platform = 'ATPS',
                   justification   = justification||' : '||'Rule13: Min Storage OK'
             WHERE db_primkey = P_DB_PRIMKEY
               AND alloc_gb >= v_limits.min_storage_gb
               AND target_platform = 'TBD';

        WHEN 14 THEN
            UPDATE mig_database
               SET target_platform = 'ATPS',
                   justification   = justification||' : '||'Rule14: Final eCPU/Memory Check'
             WHERE db_primkey = P_DB_PRIMKEY
               AND (sga_gb + pga_gb) / anz_ecpus >= v_limits.gb_mem_per_ecpu
               AND anz_ecpus > 0
               AND target_platform = 'TBD';
        ...
        WHEN 15 THEN
            UPDATE mig_database
               SET target_platform = 'EXAD',
                   justification   = justification||' : '||'Rule15: CPAT Complexity'
             WHERE db_primkey = P_DB_PRIMKEY
               AND db_primkey IN (
                   SELECT dbc_db_primkey
                     FROM mig_database_migration_classification
                    WHERE dbc_complexity_group > 2
               )
               AND target_platform = 'TBD';
        ...
        WHEN 16 THEN
            UPDATE mig_database
               SET ha_id = 'DG'
             WHERE db_primkey = P_DB_PRIMKEY
               AND target_platform IN ('ATPS', 'EXAD')
        ...
        WHEN 19 THEN
            DECLARE
                v_appl       mig_database.businessappl%TYPE;
                v_platform   mig_database.target_platform%TYPE;
                v_justif     mig_database.justification%TYPE;
                v_oracledb   mig_database.oracledb%TYPE;
            BEGIN
                -- Hole BusinessAppl, Zielplattform, Justification und DB-Name der PROD-DB
                SELECT businessappl, target_platform, justification, oracledb
                  INTO v_appl, v_platform, v_justif, v_oracledb
                  FROM mig_database
                 WHERE db_primkey = P_DB_PRIMKEY;
        
                -- Übertrage Plattform + Justification auf alle Nicht-PROD-DBs mit gleichem businessappl
                UPDATE mig_database
                   SET target_platform = v_platform,
                       justification   = 'Rule19: Copied from PROD DB ' || v_oracledb || ' (' || v_justif || ')'
                 WHERE businessappl = v_appl
                   AND environment != 'P'
                   ;
            ...
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    v_return := 'ERROR 19/' || P_DB_PRIMKEY || '/No matching PROD DB';
                    p_log('mig', v_return);
                    RETURN v_return;
        
                WHEN OTHERS THEN
                    v_return := 'ERROR 19/' || P_DB_PRIMKEY || '/' || SQLERRM;
                    p_log('mig', v_return);
                    RETURN v_return;
            END;
        ...
        WHEN 20 THEN
            UPDATE mig_database
                   SET target_platform = 'ATPS',
                       justification   = justification||' : '||'Rule20: left over for ATPS'
             WHERE db_primkey = P_DB_PRIMKEY
             and  target_platform = 'TBD'
             ;
        ...
        WHEN 21 THEN
            UPDATE mig_database
                   SET ha_id = 'DG',
                       justification   = justification||' : '||'Rule21: set DG'
             WHERE db_primkey = P_DB_PRIMKEY
             and  SERVICE_CATEGORY = 'GOLD'
             ;
...
EXCEPTION
    WHEN OTHERS THEN
        v_return := 'ERROR: ' || P_RULE_NR || '/' || P_DB_PRIMKEY || '/' || SQLERRM;
        p_log('mig', v_return);
        return v_return;
END F_EXEC_CLASS_RULE;
